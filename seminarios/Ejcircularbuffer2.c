...
//empieza aqui el ejercicio
void extract_from_circular(CircularBuffer* buffer, char* s, int numbytes){//por aqui pasaran todas, tienes que quiatr el ; y dejarlo todo bien para poder sumarlo
  for(int i=0; i<numbytes;i++){
    s[i]= buffer_pop(cb); //no con & pq ya es un puntero
  }
  s[numbytes]='\0';
  if(s[numbytes-1]==;){
    s[numbytes-1]= '\0';
...
int main(int argc, char* argv[]){
 unsigned char buffer[100]; //esto todo siempre igual
 CircularBuffer cb;
 buffer_init(&cb, 100);
 int n;
 int sum=0;
 int file =open(argv[1],O_RDONLY,0644);//abrimos el archivo para leerlo
 while((n=read(file,buffer,100))>0){//mientras lea algo lo va guardando en el buffer
    for(int i=0; i<n;i++){//recorre todo lo que ha leido
        char s[80];
        if(buffer_free_bytes==0){//si no queda espacio libre
           int nexts= buffer_size_next_element(&cb,';',0);//miramos cuanto mide lo siguiente
             if (nexts==-1){
                _exit(1);
             }
             extract_from_circular(&cb,s,nexts);
             int num= atoi(s);
             sum=sum+num;
        }
    buffer_push(&cb,buffer[i]);//si queda espacio libre metemos directamente al cb
    }
}
close(file);
    char s[80];
   while( int ne= buffer_size_next_element(&cb,';',1) != -1){
    extract_from_circular(&cb,s,ne);
    int num;
    sum=sum+num;
}

int out= open(argv[2],O_RDONLY, 0644);
write(out, &sum, sizeof(int));
close(out);
